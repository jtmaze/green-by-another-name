...
# %% 2. Use common cloud mask on Green LandSat and Sentinel2 Images 
band_color = 'green'
...
with rio.open('./data/common_mask_sieved_dilated.tif') as cmask_src:
...
with rio.open(f'./data/masked_images/{band_color}-masked-conservative.tif', 'w', **out_meta) as dst:
    dst.write(ls_data, 1)
    dst.set_band_description(1, f'landsat-{band_color}')
    dst.write(s2_data, 2)
    dst.set_band_description(2, f'sentinel2-{band_color}')

# %% 3.0 Use the common cloud mask on the NIR bands

band_color = 'nir'

ls_nir_band = 4 # NIR is the 4th band in export
s2_nir_band = 4 # NIR is the 4th band in the export
...
with rio.open('./data/sentinel2-images/sentinel2-images-merged.tif') as s2_img_src, \
    ...
    s2_data = s2_img_src.read(s2_nir_band, window=window_s2)
    ls_data = ls_img_src.read(ls_nir_band, window=window_ls)
